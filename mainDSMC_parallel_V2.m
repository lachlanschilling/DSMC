...
%               This code runs workers in parallel.
...
close all
clear global
debugMode = false;       % -     Bool for debug mode (see documentation)
printInfo = true;        % -     Bool for step output
attempt   = 1;           % -     Video file identifer                          
movie     = true;        % -     Bool for movie output
sims      = 20;          % -     Number of simulations
N_dt      = 6e3;         % -     Total number of time steps. 10000+ is good
colCELL   = [10 10 10];  % -     Number of collision cells in x y z
T0        = 293;         % K     Initial particle temperature
TW        = 1000;        % K     Wall temperature
L         = 1e-3*[1 1 1];% m     Domain size
delete(gcp('nocreate'));
pool = parpool;
% parpool(10);
%_________________________________________________________________________%
...
% INITALISE SAVE DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
savevelx   = zeros(sims, N_dt, colCELL(1), colCELL(2));
savevely   = zeros(sims, N_dt, colCELL(1), colCELL(2));
savevelz   = zeros(sims, N_dt, colCELL(1), colCELL(2));
saveT      = zeros(sims, N_dt, colCELL(1), colCELL(2));
saverho    = zeros(sims, N_dt, colCELL(1), colCELL(2));
colCELLx   = colCELL(1);
colCELLy   = colCELL(2);
%_________________________________________________________________________%
...
% SIMULATIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
parfor s = 1:sims                          % for all simulations...
	% set up current simulation
    [m, k, dL, particle, vRELMAX, Nremcol, nCELL,...
          coeffNTC, nPARTeff, samples, dt, beta, leftW,...
          rightW, bottomW, topW, frontW, backW, nPART, volCELL]...
          = initSIMU(debugMode, colCELL, T0, TW, L); 
    
    % place particles in cells
	particle = indxPART(particle,dL,nPART,debugMode,nCELL,L,colCELL); 
    
    % for all time steps...
    for steps = 1 : N_dt
        
        % advect & collide particles
        [particle, vRELMAX, Nremcol] = ...
         advcPART(dL, colCELL, nPART, particle, nCELL, L, leftW, rightW,...
                  bottomW, topW, frontW, backW, dt, beta,...
                  debugMode, coeffNTC, vRELMAX, Nremcol);
        
        % sample the data
        samples = sampCELL(k, m, volCELL, nPARTeff, particle, nCELL, ...
                            samples);
        ...
        for y = 1 : colCELLy
            for x = 1 : colCELLx
                ...
                savevelx(s, steps, x, y) = vx;
                savevely(s, steps, x, y) = vy;
                savevelz(s, steps, x, y) = vt;
                saveT(s, steps, x, y)      = T;
                saverho(s, steps, x, y)    = rh;
            end
        end
...
%_________________________________________________________________________%
saved.vel = [mean(savevelx), mean(savevely), ...
    mean(savevelz)];
saved.T   = mean(saveT);
saved.rho = mean(saverho);
% add for loop that checks if average temperature at N_dt is > 0.95*TW and
% only average those simulations. Count how many simulations meet that
% criteria
toc
delete(gcp('nocreate'));
tic
if movie
    % create a movie from saved data
    doMovie(T0, TW, L, colCELL, N_dt, saved, attempt); 
end
toc
tiledPlot(saved, N_dt, T0, TW)

% FUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [m, k, dL, particle, vRELMAX, Nremcol, nCELL,...
          coeffNTC, nPARTeff, samples, dt, beta, leftW,...
          rightW, bottomW, topW, frontW, backW, nPART, volCELL]...
          = initSIMU(debugMode, colCELL, T0, TW, L)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
particle.vel(:,1:3) = (2 * randi(2, [nPART, 3]) - 3) .* vACT/sqrt(3);
...
function particle = indxPART(particle,dL,nPART,debugMode,nCELL,L,colCELL)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILE:         indxPART.m
% AUTHOR:       Lachlan Schilling
% DATE (D/M/Y): 06/10/2022
...
function [particle, vRELMAX, Nremcol] = ...
         advcPART(dL, colCELL, nPART, particle, nCELL, L, leftW, rightW,...
                  bottomW, topW, frontW, backW, dt, beta,...
                  debugMode, coeffNTC, vRELMAX, Nremcol)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
particle = bndyCELL(particleOld, beta, dt, L,...
                    leftW, rightW, bottomW, topW, frontW, backW, ...
                    particle); % collide particles with boundary
% Reindex particles after hitting the boundaries.
particle = indxPART(particle,dL,nPART,debugMode,nCELL,L,colCELL); 
for c = 1:nCELL
    % collide particles with particles
    [vRELMAX, Nremcol, particle] = ...
    collCELL(c, coeffNTC, particle, vRELMAX, Nremcol);
end
%_________________________________________________________________________%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
function particle = bndyCELL(particleOld, beta, dt, L, leftW,...
                                      rightW, bottomW, topW, frontW,...
                                      backW, particle)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILE:         bndyCELL.m
% AUTHOR:       Lachlan Schilling
...
function [vRELMAX, Nremcol, particle] = ...
    collCELL(c, coeffNTC, particle, vRELMAX, Nremcol)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILE:         collCELL.m
% AUTHOR:       Lachlan Schilling
...
function samples = sampCELL(k, m, volCELL, nPARTeff, particle, nCELL, ...
                            samples)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILE:         sampCELL.m
% AUTHOR:       Lachlan Schilling
...
function doMovie(T0, TW, L, colCELL, N_dt, saved, attempt)
% DSMC SOLVER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FILE:         doMovie.m
% AUTHOR:       Lachlan Schilling
% DATE (D/M/Y): 07/10/2022
...
for i = 1:N_dt
    ...
    for jj = 2:colCELL(2)
        for ii = 2:colCELL(1)
            inputT(ii, jj) = 0.25 * (saved.T(1, i, ii - 1, jj -1) ...
                                   + saved.T(1, i, ii, jj -1)...
                                   + saved.T(1, i, ii, jj)...
                                   + saved.T(1, i, ii - 1, jj));
        end
...
function tiledPlot(saved, N_dt, T0, TW)
    
    % plotting
    T_av = zeros(N_dt,1);
    for i = 1:N_dt
        T_av(i) = mean(saved.T(1, i, :, :),'all');
    end
    plot(1:N_dt, T_av, 'b')
        xlabel('Time Steps')
